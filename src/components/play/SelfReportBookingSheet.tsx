        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 flex-shrink-0">
              <h2 className="text-[15px] font-bold text-ink">Report a booking</h2>
            ...
            <div className="px-5 pb-6 overflow-y-auto flex-1" style={{ paddingBottom: 'calc(32px + env(safe-area-inset-bottom))' }}>
              <p className="text-[12px] text-ink-2 mb-4">Already booked elsewhere? Let your teammates know the details.</p>

              {/* Venue */}
              {!selectedVenue && !manualMode ? (
                <div className="mb-4">
                  <label className="text-[12px] font-semibold text-ink-2 block mb-1">Venue</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      style={{ fontSize: '16px' }}
                      className="w-full rounded-xl border border-hairline pl-9 pr-4 py-2.5 outline-none focus:border-court"
                    />
                  </div>
                    ...
                    <div className="mt-1 rounded-xl border border-hairline bg-white shadow-sm overflow-hidden">
                      {venueResults.map((v) => (
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-3 py-2.5 text-left hover:bg-surface border-b border-hairline last:border-0"
                        >
                          <MapPin className="h-3.5 w-3.5 text-ink-2 flex-shrink-0" />
                          <div>
                            <p className="text-[13px] font-medium text-ink">{v.venue_name}</p>
                ...
                <div className="mb-4">
                  <button
                    ...
                    className="text-[12px] text-court-700 font-semibold mb-2 inline-flex items-center gap-1"
                  >
                    {'\u2190'} Back to search
                  </button>
                  <label className="text-[12px] font-semibold text-ink-2 block mb-1">Venue name</label>
                  <input
                    type="text"
                    ...
                    style={{ fontSize: '16px' }}
                    className="w-full rounded-xl border border-hairline px-4 py-2.5 outline-none focus:border-court"
                  />
                </div>
              ) : (
                <div className="mb-4 rounded-xl bg-court-50 border border-court-100 px-3 py-2.5 flex items-center justify-between">
                  <div>
                    <p className="text-[13px] font-semibold text-court-700">{selectedVenue!.venue_name}</p>
                    {selectedVenue!.city && <p className="text-[11px] text-court">{selectedVenue!.city}</p>}
                  </div>
                  <button onClick={() => setSelectedVenue(null)} className="text-[11px] text-court font-semibold">Change</button>
                </div>
              )}

              {/* Court number */}
              <div className="mb-4">
                <label className="text-[12px] font-semibold text-ink-2 block mb-1">Court number <span className="text-ink-2 font-normal">(optional)</span></label>
                <input
                  type="number"
                  inputMode="numeric"
                  value={courtNumber}
              ...
              {/* Booking reference */}
              <div className="mb-4">
                <label className="text-[12px] font-semibold text-ink-2 block mb-1">Booking reference <span className="text-ink-2 font-normal">(optional)</span></label>
                <input
                  type="text"
                  value={bookingRef}
              ...
              {/* Total cost */}
              <div className="mb-5">
                <label className="text-[12px] font-semibold text-ink-2 block mb-1">Total cost <span className="text-ink-2 font-normal">(optional)</span></label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-2 text-[14px]">{'\u00A3'}</span>
                  <input
                    type="number"
                    inputMode="decimal"
                    step="0.01"
                    ...
                    placeholder="0.00"
                    style={{ fontSize: '16px' }}
                    className="w-full rounded-xl border border-hairline pl-8 pr-4 py-2.5 outline-none focus:border-court"
                  />
                </div>
                {perPlayerPounds && (
                  <p className="text-[11px] text-ink-2 mt-1">= {'\u00A3'}{perPlayerPounds} each for {playerCount} players</p>
                )}
              </div>
