  ...
  if (!coach) {
    return (
      <div className="min-h-screen bg-white">
        <div className="px-5 pt-14"><button onClick={() => navigate(-1)} className="h-9 w-9 rounded-full bg-gray-100 flex items-center justify-center"><ChevronLeft className="h-5 w-5 text-gray-600" /></button></div>
        <p className="text-center text-[14px] text-gray-400 mt-10">Coach not found.</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white pb-16">
      ...
      <div className="px-5 pt-14 pb-4">
        <button onClick={() => navigate(-1)} className="h-9 w-9 rounded-full bg-gray-100 flex items-center justify-center mb-4"><ChevronLeft className="h-5 w-5 text-gray-600" /></button>
        ...
        {(coach.years_experience != null || venueNames.length > 0) && (
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-[12px] text-gray-500">
            {coach.years_experience != null && <span className="flex items-center gap-1"><Award className="h-3.5 w-3.5 text-gray-400" />{coach.years_experience}+ yrs experience</span>}
            {venueNames.length > 0 && <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-gray-400" />{venueNames.join(' · ')}</span>}
          </div>
        )}
        ...
        {Array.isArray(coach.specialties) && coach.specialties.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {coach.specialties.map((sp) => (
              <span key={sp} className="text-[11px] font-medium px-2 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-100">{sp}</span>
            ))}
          </div>
        )}

        {coach.bio && <p className="text-[14px] text-gray-600 leading-relaxed mt-4">{coach.bio}</p>}
      </div>
      ...
      <section className="px-5 mt-2">
        ...
        {sessions.length === 0 ? (
          <div className="rounded-xl bg-gray-50 border border-gray-100 p-6 text-center">
            <p className="text-[13px] text-gray-400">No sessions scheduled right now — check back soon.</p>
          </div>
        ) : (
                ...
                <div key={c.id} className="rounded-xl bg-gray-50 border border-gray-100 p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] mt-0.5">
                      {c.price_pence != null && <span className="font-semibold text-gray-700">{sym(c.currency)}{(c.price_pence / 100).toFixed(2)}</span>}
