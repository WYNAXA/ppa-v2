  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      <div className="px-5 pt-14 pb-3 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[22px] font-bold text-gray-900 leading-tight">Coaches</h1>
          <p className="text-[13px] text-gray-400">Find a coach and book a session</p>
        </div>
      </div>
        ...
        <div className="px-5 space-y-2">
          {coaches.map((c) => (
            <button
              ...
              className="w-full flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-3 text-left hover:border-teal-200 hover:bg-teal-50/20 active:scale-[0.99] transition-all"
            >
              <PlayerAvatar name={c.name} avatarUrl={c.avatar_url} size="md" />
              <div className="flex-1 min-w-0">
                <p className="text-[15px] font-bold text-gray-900 truncate">{c.name}</p>
                {c.headline && <p className="text-[12px] text-teal-600 truncate">{c.headline}</p>}
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  {c.upcoming > 0 && <span className="text-[11px] font-semibold text-emerald-600">{c.upcoming} upcoming</span>}
                  {c.venues.length > 0 && <span className="text-[11px] text-gray-400 truncate">{c.venues.slice(0, 2).join(' · ')}</span>}
                </div>
                {c.specialties.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {c.specialties.slice(0, 3).map((sp) => (
                      <span key={sp} className="text-[10px] leading-none px-1.5 py-1 rounded-full bg-teal-50 text-teal-700 border border-teal-100">{sp}</span>
                    ))}
                  </div>
                )}
              </div>
