...
--
-- A guest occupies a slot in matches.player_ids as a 'guest_<token>' placeholder;
-- their name + invite link live in match_guest_invites. When the invited person
-- signs up and opens the invite link, claim_match_guest_invite() promotes the
-- placeholder to their real profile id — so the match then counts for ELO.
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.match_guest_invites (
  ...
  slot_player_id text NOT NULL,                -- the 'guest_<token>' entry in matches.player_ids
...
USING (
  invited_by = auth.uid()
  OR EXISTS (SELECT 1 FROM public.matches m WHERE m.id = match_id AND auth.uid()::text = ANY(m.player_ids))
);
...
CREATE OR REPLACE FUNCTION public.create_match_guest_invite(
  ...
  p_replace_player_id text DEFAULT NULL      -- when replacing an existing slot; NULL = append
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE
  v_token text := encode(gen_random_bytes(9), 'hex');   -- 18 hex chars, url-safe
  v_slot  text := 'guest_' || v_token;
  v_pids  text[];
  v_id    uuid;
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE m.id = p_match_id
      AND (auth.uid()::text = ANY(m.player_ids) OR m.created_by = auth.uid())
  ) THEN
    RAISE EXCEPTION 'Not authorized to invite to this match';
  END IF;

  SELECT coalesce(player_ids, '{}') INTO v_pids FROM public.matches WHERE id = p_match_id FOR UPDATE;
  ...
  IF p_replace_player_id IS NOT NULL AND p_replace_player_id = ANY(v_pids) THEN
    ...
    -- If we replaced a guest slot, retire that old invite
  ...
  RETURN jsonb_build_object('invite_id', v_id, 'token', v_token, 'slot', v_slot);
END; $$;
...
BEGIN
  ...
  SELECT coalesce(player_ids, '{}') INTO v_pids FROM public.matches WHERE id = v_inv.match_id FOR UPDATE;

  IF v_uid::text = ANY(v_pids) THEN
    -- already a player in this match: just drop the placeholder slot
    v_pids := array_remove(v_pids, v_inv.slot_player_id);
  ELSE
    v_pids := array_replace(v_pids, v_inv.slot_player_id, v_uid::text);
  END IF;
...
GRANT EXECUTE ON FUNCTION public.create_match_guest_invite(uuid, text, text, text) TO authenticated;
