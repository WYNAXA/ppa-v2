-- discover_feed() reads visibility.
--
-- The events branch keyed on `group_id IS NULL`, which was the old proxy for
-- public. It now reads the explicit column from 20260913155650, and includes
-- 'connections' events: a friend's game night is exactly the "not yours yet"
-- thing this tab is for, and RLS already decides who counts as a connection.
--
-- GROUP events are deliberately NOT here. They already appear on Today and on
-- the group's own page; putting them in Discover as well would show one item
-- in two tabs and blur what the tab is for. PRIVATE events belong to their
-- author and appear on Today.
--
