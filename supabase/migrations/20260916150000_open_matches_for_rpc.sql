-- N8: Server-side open-match filtering. The client query was global — every
-- open match sent to every client, filtered after fetch. Same pattern that
-- produced the India/Israel venue list (M1).
--
-- SECURITY INVOKER: runs as the caller, so RLS on matches still applies.
-- No enumeration risk. The 5 SELECT policies on matches already limit
...
AS $$
  WITH candidates AS (
    ...
    WHERE m.is_open = true
      ...
      AND NOT (p_user_id = ANY(COALESCE(m.player_ids, ARRAY[]::uuid[])))
  ),
  with_venue AS (
    SELECT c.*,
