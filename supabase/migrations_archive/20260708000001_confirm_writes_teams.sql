-- ════════════════════════════════════════════════════════════════════════════
-- Add team1_player_ids / team2_player_ids to confirm_poll_schedule INSERT.
-- The columns already exist on matches (20260508000003). This migration
-- only updates the RPC to pass them through from p_schedule.
--
-- Run each block separately in the SQL Editor.
-- ════════════════════════════════════════════════════════════════════════════


-- 1. Updated confirm_poll_schedule — now reads team columns from schedule JSON
...
BEGIN
  ...
  LOOP
    ...
    -- Team columns: nullable. Only set if the engine computed a balanced split.
    v_team1 := NULL;
    v_team2 := NULL;
