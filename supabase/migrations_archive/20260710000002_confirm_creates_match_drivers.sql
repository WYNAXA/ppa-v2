-- ════════════════════════════════════════════════════════════════════════════
-- Update confirm_poll_schedule to auto-create match_drivers rows for players
-- who answered "I can drive" in their poll response.
--
-- SAME 3-arg signature: (uuid, jsonb, uuid[])
-- Atomic: match_drivers inserts happen in the same transaction as match creation.
-- ON CONFLICT DO NOTHING: idempotent if re-run.
--
-- Run each block separately in the SQL Editor.
-- ════════════════════════════════════════════════════════════════════════════


-- 1. Updated confirm_poll_schedule — adds match_drivers for poll drivers
...
BEGIN
  ...
  LOOP
    ...
    -- Auto-create match_drivers for players who answered "I can drive" in their poll response.
    -- Atomic with match creation. ON CONFLICT DO NOTHING for idempotency.
    INSERT INTO match_drivers (match_id, driver_id, seats_available)
...
-- 4. Verify match_drivers reference exists in the function
-- Run after applying:
-- SELECT position('match_drivers' in prosrc) > 0 AS has_match_drivers
