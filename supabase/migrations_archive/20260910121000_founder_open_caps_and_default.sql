-- === FILE: supabase/migrations/20260910121000_founder_open_caps_and_default.sql ===
--
-- Founder caps become OPTIONAL (null = open, no limit). Adds a '*' global-default program
-- so any venue can be tagged founder before a per-country row exists. grant_founder_contract
-- falls back to '*' and only enforces a cap when one is set. admin_founder_slots hides '*'
-- and shows null cap/remaining as "open". No pricing-path or live-contract changes.

alter table public.founder_programs alter column cap drop not null;

update public.founder_programs set cap = null where country_code in ('GB','IT');

insert into public.founder_programs (country_code, cap, plan_tier, waive_fee, commission_rate_bps, term_months)
  values ('*', null, 'pro', true, 225, 12)
